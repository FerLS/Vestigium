...
class Gravedigger(pygame.sprite.Sprite):
    def __init__(self, x, y, tilemap):
        ...
        self.walk_sheet = self.resource_manager.load_image("enemy-walk.png", "assets\\enemy")
        self.idle_sheet = self.resource_manager.load_image("enemy-idle.png", "assets\\enemy")
        
        self.tilemap = tilemap
        self.rect = pygame.Rect((0, 0), (42, 47))
        self.light = Light(x + 20, y, "assets/enemy/lantern.png") # Asociamos light ao farolillo
    ...
    def animate(self):
        ...
        if self.animationCounter >= self.animationSpeed:
            self.animationCounter = 0
            self.imagePosture = (self.imagePosture + 1) % len(self.walk_frames if self.state == MovementType.WALK else self.idle_frames)
    ...
    def move(self):
        if self.state == MovementType.WALK:
            ...
            self.time -= 1
            if self.time <= 0:
                self.movement = random.choice([MovementDirections.LEFT, MovementDirections.RIGHT])
    ...
    def collide(self, player):
        if self.mask.overlap(player.mask, (player.rect.x - self.rect.x, player.rect.y - self.rect.y)):
            if (self.movement == MovementDirections.RIGHT and player.rect.x < self.rect.x) or (self.movement == MovementDirections.LEFT and player.rect.x > self.rect.x):
                player.get_key()
                self.stop()
    ...
    def update(self, player, camera_scroll):
        self.start(player)
        self.collide(player)
        self.move()
        self.animate()
    ...
    def draw(self, screen, camera_scroll):
        ...
        screen.blit(img, (self.rect.x - camera_scroll, self.rect.y))
